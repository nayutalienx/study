public class Plant implements Comparable <Plant>
{
    ...
    public boolean accord(Region region){
        boolean tempResult = (minTemp >= region.getMinYearTemp() && minTemp <= region.getMaxYearTemp()) &&
                             (maxTemp >= region.getMinYearTemp() && maxTemp <= region.getMaxYearTemp());
        boolean rainResult = (requirement == "Интенсивное" && region.getRainyDays() >= 250) ||
                (requirement == "Среднее" && region.getRainyDays() < 250 && region.getRainyDays() >= 150) ||
                (requirement == "Редкое" && region.getRainyDays() < 150 && region.getRainyDays() >= 50);
